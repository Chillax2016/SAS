...
plot dowjoneshigh*year
	/haxis = 1955 to 1995 by 5 
	 vaxis =0 to 6000 by 1000
	 hminor = 3 /*HMINOR= 和 VMINOR= 指定主刻度标之间的刻度标数目。*/
	 vminor = 1
	 vref   = 1000 3000 5000
	 lvref  = 2 /*LVREF= 为参考线指定线型（虚线）；*/
	 cvref  = blue /*CVREF= 指定兰色为线条的颜色。*/
	 caxis  = blue /*CAXIS= 为轴线和所有主副刻度标指定颜色。*/
	 ctext  = red; /*CTEXT= 将所有点/线图文本（包括轴标签和主刻度标值）指定为红色。*/
symbol1 color = red
		interpol = join /*INTERPOL=JOIN 表示用直线连接数据点。*/
		value = dot
		height =1;
